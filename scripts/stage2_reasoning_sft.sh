...
#
# Goal:
#   Jointly improve reasoning text + [SEG] hidden representation.
#
# Train:
#   - Qwen LLM LoRA
#   - ReaSegProjector
...
#   so mask loss can update LLM LoRA through [SEG] hidden state.
# ============================================================
...
export OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/outputs/stage2_reasoning_sft}"

# Resume from Stage 1.
export RESUME_FROM_CHECKPOINT="${RESUME_FROM_CHECKPOINT:-${PROJECT_ROOT}/outputs/stage1_projector_warmup/final_model}"

# New stage: usually do NOT resume optimizer/scheduler states.
...
# -----------------------------
# Schedule
# For 1300 samples: 5 epochs.
# For 45k samples: 1-3 epochs.
# -----------------------------
...
# Let mask loss update LLM LoRA through [SEG].
export DETACH_SEG_HIDDEN_FOR_MASK=0
...
# -----------------------------
# Loss weights
# Dice should be more important for BraTS foreground overlap.
# -----------------------------
...
if [[ ! -d "${RESUME_FROM_CHECKPOINT}" ]]; then
  ...
  echo "Run scripts/stage1_projector_warmup.sh first, or set RESUME_FROM_CHECKPOINT manually."
  exit 1
fi
